...
def test_edge_index_k_weighted(simple_temporal_graph):
    ...
    assert equal(e1, IntTensor([
        [0, 1, 2, 2],
        [1, 2, 3, 4]
        ]).to(config['torch']['device'])) # a -> b | b -> c | c -> d | c -> e

    assert equal(w1, tensor([1., 1., 1., 1.]).to(config['torch']['device']))

    e2, w2 = PathData.edge_index_k_weighted(paths, k=2)
    assert equal(e2, IntTensor([
        [[0, 1],
         [1, 2],
         [1, 2]],
        [[1, 2],
         [2, 3],
         [2, 4]]
         ]).to(config['torch']['device'])) # a-b -> b-c | b-c -> c-d | b-c -> c-e

    assert equal(w2, tensor([1., 1., 1.]).to(config['torch']['device']))

    e3, w3 = PathData.edge_index_k_weighted(paths, k=3)
    assert equal(e3, IntTensor([
        [[0, 1, 2],
         [0, 1, 2]],
        [[1, 2, 3],
         [1, 2, 4]]
    ...
    assert equal(w3, tensor([1., 1.]).to(config['torch']['device'])) # a-b-c -> b-c-d | a-b-c -> b-c-e
