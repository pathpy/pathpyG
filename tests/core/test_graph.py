...
def test_init_with_list_mapping():
    edge_index = get_random_edge_index(100, 100, 1000)
    data = Data(edge_index=edge_index, num_nodes=100)
    idxs = [str(i) for i in range(100)]
    g = Graph(data, idxs)
...
def test_from_edge_list_fails():
    ...
    with pytest.raises(ValueError):
        Graph.from_edge_list(directed_edge_list, is_undirected=True)
    ...
    with pytest.raises(ValueError):
        Graph.from_edge_list(undirected_edge_list, is_undirected=False)
