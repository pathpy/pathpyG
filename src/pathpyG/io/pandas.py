...
def graph_to_df(graph: Graph, node_indices: Optional[bool] = False) -> pd.DataFrame:
    ...
    edge_attribute_df = pd.DataFrame.from_dict(
        {a: graph.data[a] for a in graph.edge_attrs()})
    df = pd.concat([df, edge_attribute_df], axis=1)
    return df


def temporal_graph_to_df(graph: TemporalGraph, 
                         node_indices: Optional[bool] = False) -> pd.DataFrame:
...
def read_csv_graph(filename: str,
                sep: str = ',',
                header: bool = True,
                is_undirected: bool = False,
                multiedges: bool = False,
...
def read_csv_temporal_graph(filename: str,
                 sep: str = ',',
                 header: bool = True,
