...
def df_to_graph(df: pd.DataFrame, loops: bool = True, directed: bool = True,
               multiedges: bool = False, **kwargs: Any) -> Graph:
    ...
    **kwargs: Any
    ...
    >>> n = pp.io.df_to_graph(df, name='pandasGraph')
    >>> print(n)
    """
    ...
    if None in node_set:
        print('DataFrame minimally needs columns \'v\' and \'w\'')
        raise IOError
...
def add_edge_attributes(df: pd.DataFrame, g: Graph):
...
def df_to_temporal_graph(df: pd.DataFrame, timestamp_format='%Y-%m-%d %H:%M:%S', time_rescale=1,
                        **kwargs: Any) -> TemporalGraph:
...
def to_df(graph: Union[Graph, TemporalGraph], node_indices: Optional[bool] = False) -> pd.DataFrame:
    """Returns a pandas dataframe of a given graph.
...
def read_csv(filename: str,
                 ...
                 time_rescale: int = 1,
                 **kwargs: Any) -> Union[Graph,TemporalGraph]:
    ...
    if 't' in df:
        return df_to_temporal_graph(df, timestamp_fromat=timestamp_format, time_rescale=time_rescale, **kwargs)
    else:
        return df_to_graph(df, loops=loops, directed=directed, multiedges=multiedges, **kwargs)


def write_csv(graph: Union[Graph, TemporalGraph],
          path_or_buf: Any = None,
          node_indices: bool = False,
          **pdargs: Any) -> None:
    """Stores all edges including edge attributes in a csv file."""
    frame = to_df(graph=graph, node_indices=node_indices)
