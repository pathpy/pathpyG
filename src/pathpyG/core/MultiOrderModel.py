...
class MultiOrderModel:
    ...
    def get_intermediate_order_log_likelihood(self, dag_graph: DataBatch, order: int) -> float:
        ...
        return (
            torch.mul(
                frequencies,
                torch.log(
                    compute_transition_probabilities(self.layers[order])[
    ...
    def get_mon_log_likelihood(self, dag_graph: DataBatch, max_order: int = 1) -> float:
        ...
        if max_order > 0:
            llh += (
                (
                    torch.log(compute_transition_probabilities(self.layers[max_order]))
                    * self.layers[max_order].data.edge_weight
                )
                .sum()
                .item()
            )
