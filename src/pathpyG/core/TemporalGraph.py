...
class TemporalGraph(Graph):
    def __init__(self, data: TemporalData, mapping: IndexMap = None) -> None:
        ...
        self.data = TemporalData(src=data.src[sort_index], dst=data.dst[sort_index], t=t_sorted)
    ...
    def from_edge_list(edge_list) -> TemporalGraph:
        ...
        return TemporalGraph(
            data=TemporalData(src=torch.Tensor(sources).long(), dst=torch.Tensor(targets).long(), t=torch.Tensor(ts)),
            mapping=index_map,
        )
    ...
    def get_window(self, start: int, end: int) -> TemporalGraph:
        ...
        #     )
        return TemporalGraph(
            data=TemporalData(src=self.data.src[start:end], dst=self.data.dst[start:end], t=self.data.t[start:end]),
