...
import torch 
import torch.nn import Linear, ModuleList
...
class DBGNN(torch.nn.Module):
    ...
    def __init__(
        ...
        # higher-order layers
        self.higher_order_layers = ModuleList(GCNConv(self.num_features[1], self.hidden_dims[0]))

        # first-order layers
        self.first_order_layers = ModuleList(GCNConv(self.num_features[1], self.hidden_dims[0]))
    ...
    def forward(self, data):
        ...
        x = torch.nn.functional.elu(self.layer_2_1((x_h, x), data.bipartite_edge_index, N = data.num_ho_nodes, M= data.num_nodes))
        x = F.dropout(x, p=self.p_dropout, training=self.training)
